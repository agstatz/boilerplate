...
exports.getFoodRating = async (req, res) => {
    ...
    try {
        FoodRating.findOne({ownerName: queryUser,
            food: queryFood}).exec((err, foodRating) => {
                if (err) {
                    res.status(500).send({ message: err });
                    return;
                }
                if (!foodRating) {
                    console.log("eror1");
                    FoodRating.create({ ownerName: queryUser, food: queryFood, rating: 0 }, function (err, newFoodRating) {
