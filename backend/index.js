...
// internal config
const url =
  "mongodb+srv://boilerplate:boilerPlate407!!@cluster0.ggera.mongodb.net/myFirstDatabase?retryWrites=true&w=majority"; // Change url as needed, this is default if hosting locally
  ...
  MongoClient.connect(url, function (err, dbt) {
    ...
    db.collection("foods")
      ...
      .toArray(function (err, result) {
        ...
        ); //filters foods accordingly to search
          ...
          if (include.includes("Total Fat")) {
            if (food[1] == null || food[1][0] === "0") {
              if (result.indexOf(food[0]) !== -1)
                result.splice(result.indexOf(food[0]), 1);
            }
          }
...
app.get("/Picture", (req, res) => {
  ...
  res.sendFile(
    __dirname + "\\data\\Pictures\\" + req.query.picturename + ".jpg"
  );
          ...
          db.collection("foods")
            ...
            .toArray(function (err, result) {
              ...
              if (result.length === 0) {
                ...
                ret = db
                  .collection("foods")
                  .updateOne({ name: req.query.name }, updateDoc);
                res.send("success");
              } else {
    ...
    db.collection("users")
      ...
      .toArray(function (err, result) {
        ...
        if (result.length === 0) {
          ...
          db.collection("foods")
            .find({ name: req.query.name.split("_").join(" ") })
            .toArray(function (err, result) {
              if (err) throw err;
              if (result.length === 0) {
                res.send("Food does not exist");
                ...
                if (req.query.changeFrom === "true") {
                  if (
                    result.indexOf(req.query.name.split("_").join(" ")) !== -1
                  ) {
                    result.splice(
                      result.indexOf(req.query.name.split("_").join(" ")),
                      1
                    );
                  }
                }
...
try {
  if (requireDatabase) {
    MongoClient.connect(url, function (err, dbt) {
      ...
      db.createCollection("reply_reports", function (err, res) {
        dbt.close();
      });
    });
  }
    ...
    MongoClient.connect(url, function (err, dbt) {
      ...
      db.collection("users")
        ...
        .toArray(function (err, res) {
          fs.writeFileSync("./data/users.json", JSON.stringify(res));
        });
      db.collection("comment_reports")
        .find({})
        .toArray(function (err, res) {
          fs.writeFileSync("./data/comment_reports.json", JSON.stringify(res));
        });
      db.collection("comments")
        .find({})
        .toArray(function (err, res) {
          fs.writeFileSync("./data/comments.json", JSON.stringify(res));
        });
      db.collection("course_schedules")
        .find({})
        .toArray(function (err, res) {
          fs.writeFileSync("./data/course_schedules.json", JSON.stringify(res));
        });
      db.collection("food_categories")
        .find({})
        .toArray(function (err, res) {
          fs.writeFileSync("./data/food_categories.json", JSON.stringify(res));
        });
      db.collection("foods")
        .find({})
        .toArray(function (err, res) {
          fs.writeFileSync("./data/foods.json", JSON.stringify(res));
        });
      db.collection("intake_plans")
        .find({})
        .toArray(function (err, res) {
          fs.writeFileSync("./data/intake_plans.json", JSON.stringify(res));
        });
      db.collection("locations")
        .find({})
        .toArray(function (err, res) {
          fs.writeFileSync("./data/locations.json", JSON.stringify(res));
        });
      db.collection("meal_plans")
        .find({})
        .toArray(function (err, res) {
          fs.writeFileSync("./data/meal_plans.json", JSON.stringify(res));
        });
      db.collection("replies")
        .find({})
        .toArray(function (err, res) {
          fs.writeFileSync("./data/replies.json", JSON.stringify(res));
        });
      db.collection("misc")
        .find({})
        .toArray(function (err, res) {
          fs.writeFileSync("./data/misc.json", JSON.stringify(res));
        });
      db.collection("tags")
        .find({})
        .toArray(function (err, res) {
          fs.writeFileSync("./data/tags.json", JSON.stringify(res));
        });
      db.collection("reply_reports")
        .find({})
        .toArray(function (err, res) {
          fs.writeFileSync("./data/reply_reports.json", JSON.stringify(res));
        });
      db.collection("universities")
        .find({})
        .toArray(function (err, res) {
          fs.writeFileSync("./data/universities.json", JSON.stringify(res));
        });
      db.collection("visits")
        .find({})
        .toArray(function (err, res) {
          fs.writeFileSync("./data/visits.json", JSON.stringify(res));
          dbt.close();
        });
    });
  }
...
function importTables() {
  if (importJSONs) {
    ...
    MongoClient.connect(url, function (err, dbt) {
      ...
      data = JSON.parse(
        fs.readFileSync("./data/comment_reports.json").toString()
      );
      ...
      data = JSON.parse(
        fs.readFileSync("./data/course_schedules.json").toString()
      );
      db.collection("course_schedules").insertMany(
        data,
        function (err, res) {}
      );
      data = JSON.parse(
        fs.readFileSync("./data/food_categories.json").toString()
      );
      ...
      data = JSON.parse(
        fs.readFileSync("./data/reply_reports.json").toString()
      );
      ...
      db.collection("visits").insertMany(data, function (err, res) {
        dbt.close();
      });
