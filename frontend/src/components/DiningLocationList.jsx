...
function DiningLocationList(props: Props) {
    ...
    useEffect( async() => {
        ...
        try {
            const {data:response} = await axios.get("http://localhost:3001/api/dining-locations");
    ...
    function hiddenFormatter(cell, row) {
        return (
            <Form.Check checked={ cell } readonly="true"/>
        );
    }
    ...
    function idFormatter(cell, row) {
        return (
            <Link to={`/dining-courts/${row.name}`}>{row._id}</Link>
        );
    }
