...
function UserTagsList(props: Props) {
    ...
    useEffect(async () => {
        try {
            ...
            const { data: response } = await axios.get(
                'http://localhost:3001/api/foods/getuserfoodtags/'.concat(
                    targetFood.replace(/_/g, ' ')
                )
            );
    ...
    if (!emptyResponse) {
        return (
            <Container>
                <div className='ovr row flex-col flex-nowrap pb-1'>
                    {foodTagArray.map((foodTagArray, k) => (
                        <div className='col flex-start'>
                            <div className='tag-block flex-start p-3'>
        ...
        return (
            <Container>
                <div>
                    <p className='user-tag-empty pt-2'>
                        There are no food tags for this food item. Be the first
                        to create one!
                    </p>
                </div>
            </Container>
