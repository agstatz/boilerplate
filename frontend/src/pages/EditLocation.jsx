...
class EditLocation extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      newName: props.location.pathname.substring(
        props.location.pathname.lastIndexOf("/") + 1
      ),
      diningLocUrl: "/dining-courts/".concat(
  ...
  async callAPI() {
    ...
    try {
      response = await axios.get(
        url.concat("api/dining-locations/").concat(this.state.locationName)
      );
    } catch (error) {
      console.log("error");
    } finally {
  ...
  handleSubmit = (event) => {
    ...
    if (
      noErr &&
      !(this.state.newName.valueOf() == this.state.locationName.valueOf())
    ) {
      axios
        .get(
          "http://localhost:3001/api/dining-locations/".concat(
        ...
        >
          <div className="p-5 my-4 mx-3  d-flex justify-content-center bg-light border rounded">
            <Stack>
              ...
              <Stack spacing={4}>
                <a href={this.state.diningLocUrl} align="center">
                  Return to dining location page
                </a>
              </Stack>
              <Stack spacing={4}>
              ...
              >
                ...
                <Modal.Body>
                  You are about to permanently delete this dining location. Are
                  you sure you want to do this?
                </Modal.Body>
                <Modal.Footer>
                  <Button variant="primary" onClick={this.handleSubmitDelete}>
                    Confirm Delete
                  </Button>
