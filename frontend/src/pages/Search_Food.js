...
export default class Search_Food extends React.Component {
      ...
      try {
        ...
        for (let i = 0; i < this.state.nutrition.data.length; i++) {
          clonedToPush.props.children.push(
            <option value={this.state.nutrition.data[i]}>
              {this.state.nutrition.data[i]}
            </option>
          );
        }
        ...
        <Container style={{ paddingTop: "12vh", paddingBottom: "8vh" }}>
          <header className="p-3 my-4 mx-4 bg-light border rounded">
            ...
            >
              ...
              <div class={"input-group"}>{listItems3}</div>
            </div>
            <div
              style={{
                textAlign: "center",
  ...
  handleDrop2 = (event) => {
    ...
    let string = this.state.html3.find(
      (a) => a.props.id == "nutrition" + event.target.id.substring(4)
  ...
  addButton = (event) => {
    if (this.state.onList.indexOf(this.state.nutritionSelected) == -1) {
      ...
      let toPush = (
        <div class={"input-group"} id={"nutrition" + this.state.index}></div>
      );
      let clonedToPush = React.cloneElement(toPush, { children: [] });
      clonedToPush.props.children.push(
        <select
          class="form-select w-25"
          id={"drop" + this.state.index}
          onChange={this.handleDrop2}
        >
      ...
      clonedToPush.props.children.push(
        <a class="btn btn-static w-50 text-left">
          {this.state.nutritionSelected}
        </a>
      );
      clonedToPush.props.children.push(
        <Button value={this.state.index} onClick={this.removeButton}>
          Remove
        </Button>
      );
  ...
  removeButton = (event) => {
    let string = this.state.html3.pop({ id: "drop" + event.target.value }).props
